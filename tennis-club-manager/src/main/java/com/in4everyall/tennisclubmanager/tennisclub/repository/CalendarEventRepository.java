...
public interface CalendarEventRepository extends JpaRepository<CalendarEventEntity, UUID> {
    ...
    // Incluye fetch join de user para reservas (necesario para mostrar información del usuario cuando es ADMIN)
    ...
    // Buscar reservas de una pista en un rango de fechas (solo RESERVATIONS, no CLASSES)
    // Con fetch join para cargar relaciones (court y user) necesarias para mostrar información del usuario
    @Query("SELECT e FROM CalendarEventEntity e " +
    ...
    // Buscar eventos que solapen con un rango de tiempo (para validar disponibilidad de reservas)
    ...
    // Buscar clases que solapen con un rango de tiempo (las clases bloquean todas las pistas)
    // Busca clases que NO tienen court_id (court IS NULL) o que tienen court_id explícitamente NULL
    @Query("SELECT e FROM CalendarEventEntity e " +
    ...
    // Método alternativo: buscar todas las clases del día sin filtrar por solapamiento
    // Útil para debugging
    @Query("SELECT e FROM CalendarEventEntity e " +
           "WHERE e.eventType = 'CLASS' " +
           "AND e.status != 'CANCELLED' " +
